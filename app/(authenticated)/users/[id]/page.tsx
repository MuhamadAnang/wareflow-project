...
export default function UserPage() {
    ...
    const primaryEmailAddress = user?.emailAddresses?.find(email => email.id === user.primaryEmailAddressId)?.emailAddress ?? null;
    const googleEmailAddress = user?.externalAccounts?.find(account => account.provider === "oauth_google")?.emailAddress ?? null;
                ...
                <TableBody>
                    ...
                    <TableRow>
                        ...
                        <TableCell>
                            ...
                            {!isBanned && !isAllowedToLogin && <Badge variant="warning">Menunggu Persetujuan</Badge>}
                            {isBanned && <Badge variant="destructive" className="ml-2">Diblokir</Badge>}
                        </TableCell>
                    </TableRow>
                    <TableRow>
                        <TableCell className="font-medium">Dibuat Pada</TableCell>
                        <TableCell>{user?.createdAt ? format(fromUnixTime(user?.createdAt), "dd MMM yyyy HH:mm:ss") : "-"}</TableCell>
                    </TableRow>
                    <TableRow>
                        <TableCell className="font-medium">Terakhir Diperbarui</TableCell>
                        <TableCell>{user?.updatedAt ? format(fromUnixTime(user?.updatedAt), "dd MMM yyyy HH:mm:ss") : "-"}</TableCell>
                    </TableRow>
                    <TableRow>
                        <TableCell className="font-medium">Terakhir Login</TableCell>
                        <TableCell>{user?.lastSignInAt ? format(fromUnixTime(user?.lastSignInAt), "dd MMM yyyy HH:mm:ss") : "-"}</TableCell>
                    </TableRow>
                    <TableRow>
                        <TableCell className="font-medium">Terakhir Aktif</TableCell>
                        <TableCell>{user?.lastActiveAt ? format(fromUnixTime(user?.lastActiveAt), "dd MMM yyyy HH:mm:ss") : "-"}</TableCell>
                    </TableRow>
                </TableBody>
            </Table>
        </Page>
