...
export const PriorityTable = ({ priorities, isLoading }: PriorityTableProps) => {
  ...
  // Tentukan apakah perlu scroll (jika lebih dari 5 item)
  const needScroll = priorities.length > 5;

  return (
    <div className={needScroll ? "overflow-auto max-h-[500px]" : ""}>
      <Table>
        <TableHeader className={needScroll ? "sticky top-0 bg-white" : ""}>
          <TableRow>
