...
//   const existing = await getPurchaseOrderByIdService(id).catch(() => null);
//   if (!existing) {
//     throw new NotFoundException(`Purchase Order dengan ID ${id} tidak ditemukan`);
//   }
...
//   // Check existence
//   const existing = await getPurchaseOrderByIdService(id);
//   if (!existing) {
//     throw new NotFoundException(`Purchase Order dengan ID ${id} tidak ditemukan`);
//   }
...
//     note: data.note || null,
//   };
//   const [updatedHeader] = await updatePurchaseOrderHeaderRepository(id, updateData);
