...
import "server-only";
import {
  approveOrRejectClerkUserService,
  createClerkUserService,
  deleteClerkUserService,
  getClerkUserByIdService,
  getClerkUsersService,
  updateClerkUserService,
} from "./clerk.service";
import { responseFormatter } from "@/lib/response-formatter";
import { NotFoundException } from "@/common/exception/not-found.exception";
import {
  ActionApprovalUserSchema,
...
export const createClerkUserController = async (request: NextRequest) => {
  try {
    ...
    const { data } = validateSchema<TCreateOrUpdateClerkUserSchema>(
      CreateOrUpdateClerkUserSchema,
      body,
    );
